...
export class PenjualEditComponent implements OnInit {
  ...
  sellereditform(){
    this.Selleredit = this.fb.group({
      ...
      selphone:['',Validators.compose([Validators.pattern('^([0][8]|[+][6][2])[- +()0-9]+'),Validators.required, Validators.minLength(10), Validators.maxLength(14)])],
      selfax:['',Validators.compose([Validators.pattern('^[- +()0-9]+'),Validators.required,Validators.minLength(7), Validators.maxLength(15)])],
     
    })
  }

  constructor(private fb: FormBuilder,private route: ActivatedRoute, private penjualserv: PenjualDataService, private router: Router) 
  { this.sellereditform();}
  ...
  updateSeller(sellereditform:any) {
    if (sellereditform.valid){
      ...
      this.penjualserv.updateSeller(this.curseller.id, newsel)
        .subscribe(
          sel => {
